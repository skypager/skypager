...
const webConfig = merge.strategy({ entry: 'replace' })(baseProductionConfig, {
  name: 'web',
  resolve: {
    alias: {
...
const minifiedWebConfig = merge.strategy({ entry: 'replace' })(baseProductionConfig, {
  name: 'web',
  resolve: {
    alias: {
      lodash: require.resolve('lodash/lodash.min.js'),
...
const nodeConfig = merge(baseCommonConfig, {
  target: 'node',
  name: 'node',
  devtool: 'source-map',
  entry: {
...
// shouldn't be necessary as minification is opt in
