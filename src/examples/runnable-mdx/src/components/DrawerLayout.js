...
export default class DrawerLayout extends Component {
  ...
  componentWillMount() {
    ...
    workspace.drawers.observe(({ name, newValue }) => {
      this.setState(current => ({
        ...current,
        visibility: { ...current.visibility, [name]: !!newValue.visible },
      }))
    })
  }
  ...
  render() {
    ...
    return (
      <Sidebar.Pushable as={Container} fluid style={{ height: '100%' }} {...containerProps}>
        {sidebars.map(
          ({
            id,
            direction = id,
            ...
            width = '60%',
            height = '50% !important',
            styles = sidebarStyles[id] || {},
            innerStyles = innerSidebarStyles[id],
            ...props
          }) => {
            return (
              <Sidebar
