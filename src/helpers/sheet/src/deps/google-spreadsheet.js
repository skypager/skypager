...
var GoogleSpreadsheet = function(ss_key, auth_id, options) {
  ...
  this.setAuth = function(username, password, cb) {
    return cb(
      new Error(
        'Google has officially deprecated ClientLogin. Please upgrade this module and see the readme for more instrucations'
      )
    )
  }
  ...
  this.useServiceAccountAuth = function(creds, cb) {
    if (typeof creds === 'string') {
      try {
        creds = __non_webpack_require__(creds)
      } catch (err) {
        return cb(err)
      }
    }
  ...
  this.removeWorksheet = function(sheet_id, cb) {
    ...
    self.makeFeedRequest(
      GOOGLE_FEED_URL + 'worksheets/' + ss_key + '/private/full/' + sheet_id,
      'DELETE',
      null,
      cb
    )
  ...
  this.addRow = function(worksheet_id, data, cb) {
    ...
    data_xml += '</entry>'
    self.makeFeedRequest(['list', ss_key, worksheet_id], 'POST', data_xml, function(
      err,
      data,
      new_xml
    ) {
...
var SpreadsheetWorksheet = function(spreadsheet, data) {
  ...
  function _setInfo(opts, cb) {
    ...
    var xml =
      ...
      '</title>' +
      '<gs:rowCount>' +
      (opts.rowCount || self.rowCount) +
      '</gs:rowCount>' +
      '<gs:colCount>' +
      (opts.colCount || self.colCount) +
      '</gs:colCount>' +
      '</entry>'
  ...
  this.bulkUpdateCells = function(cells, cb) {
    ...
    spreadsheet.makeFeedRequest(self['_links']['bulkcells'], 'POST', data_xml, function(err, data) {
      ...
      if (data.entry && data.entry.length)
        data.entry.forEach(function(cell_data) {
  ...
  this.setHeaderRow = function(values, cb) {
    ...
    if (values.length > self.colCount) {
      return cb(
        new Error(
          'Sheet is not large enough to fit ' + values.length + ' columns. Resize the sheet first.'
        )
      )
    }
    self.getCells(
    ...
    */
    ...
    Object.keys(self).forEach(function(key) {
      if (key.substr(0, 1) != '_' && typeof (self[key] == 'string')) {
        data_xml = data_xml.replace(
          new RegExp(
            '<gsx:' + xmlSafeColumnName(key) + '>([\\s\\S]*?)</gsx:' + xmlSafeColumnName(key) + '>'
          ),
          '<gsx:' +
            xmlSafeColumnName(key) +
            '>' +
            xmlSafeValue(self[key]) +
            '</gsx:' +
            xmlSafeColumnName(key) +
            '>'
        )
...
var SpreadsheetCell = function(spreadsheet, worksheet_id, data) {
  ...
  self.save = function(cb) {
    ...
    var edit_id =
      'https://spreadsheets.google.com/feeds/cells/key/worksheetId/private/full/R' +
      self.row +
      'C' +
      self.col
    var data_xml =
